...
const LOGS_CONCURRENCY = Math.max(
  1,
  Math.min(8, parseInt(process.env.LOGS_CONCURRENCY || "6", 10) || 6)
);
...
async function withRpcRotation(fn) {
  let lastErr = null;
  // Try a few times, rotating RPCs. Small backoffs on 429/5xx.
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    try {
      ...
      if (status && status >= 500) {
        await sleep(160 + attempt * 90);
        continue;
      }

      // Short retry for transient network / upstream issues
      if (attempt < 2) {
        await sleep(120);
        continue;
      }
