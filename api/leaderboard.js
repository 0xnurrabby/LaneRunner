...
function decodePointsAndWeek(payloadHex) {
  ...
  // Optional third word: Farcaster fid (uint256)
  let fid = null;
  if (hex.length >= 192) {
    try {
...
// Fetch Farcaster usernames by fid (more reliable than address mapping when tx sender is a Smart Account)
async function fetchNamesFromNeynarFids(fids) {
  const key = process.env.NEYNAR_API_KEY;
  if (!key) return new Map();

  const uniq = [...new Set((fids || []).map((f) => String(f || "")).filter(Boolean))];
  if (!uniq.length) return new Map();
  ...
  for (let i = 0; i < uniq.length; i += chunkSize) {
    ...
    for (const u of j?.users || []) {
      ...
      out.set(String(u.fid), u.username);
    }
  }
...
module.exports = async function handler(req, res) {
  try {
    ...
    for (let from = fromBlock; from <= latest; ) {
      ...
      for (const l of logs) {
        ...
        if (fid !== null && fid !== undefined) {
          ...
          if (k && !addrToFid.has(k)) addrToFid.set(k, fid);
        }
    ...
    const topFids = [...new Set(topAddrs.map((a) => {
      const fid = addrToFid.get(String(a).toLowerCase());
      return fid != null ? String(fid) : null;
    }).filter(Boolean))];

    const [basenameMap, ensMap, fcAddrMap, fcFidMap] = await Promise.all([
      fetchBasenamesOnBase(topAddrs),
      fetchEnsPrimaryFromEnsData(topAddrs),
      fetchNamesFromNeynar(topAddrs),
      fetchNamesFromNeynarFids(topFids)
    ]);

    function pickName(addr) {
      const k = String(addr || "").toLowerCase();

      // 0) Farcaster fid (most reliable in Farcaster hosts)
      const fid = addrToFid.get(k);
      if (fid != null) {
        const u = fcFidMap.get(String(fid));
