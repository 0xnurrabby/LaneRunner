...
function decodePointsWeekAndFid(payloadHex) {
  ...
  if (hex.length >= 192) {
    try {
      fid = BigInt("0x" + hex.slice(128, 192));
    } catch {
      fid = 0n;
    }
  }
  return { points, week, fid };
}
...
async function fetchUsersByFids(fids) {
  ...
  for (let i = 0; i < uniq.length; i += chunkSize) {
    ...
    const url =
      ...
      new URLSearchParams({ fids: chunk.join(",") });
    ...
    for (const u of users) {
      ...
      const username = typeof u?.username === "string" ? u.username.replace(/^@/, "") : null;
      if (Number.isFinite(fid) && fid > 0 && username) out.set(fid, username);
    }
  }
...
module.exports = async function handler(req, res) {
  try {
    ...
    const fidByAddr = new Map(); // addr(lowercase) -> fid (from v2 payload)
    ...
    for (let from = fromBlock; from <= latest; ) {
      ...
      for (const l of logs) {
        ...
        const dec = decodePointsWeekAndFid(payload);
        if (!dec) continue;

        const points = dec.points;
        const wk = dec.week;
        const fid = dec.fid || 0n;
        if (fid > 0n) fidByAddr.set(user, fid);
    ...
    const topFids = topAddrs
      .map((a) => {
        const f = fidByAddr.get(String(a).toLowerCase()) || 0n;
        const n = Number(f);
        return Number.isFinite(n) ? n : 0;
      })
      .filter((n) => n > 0);

    const [basenameMap, ensMap, fcMap, fidUserMap] = await Promise.all([
      fetchBasenamesOnBase(topAddrs),
      fetchEnsPrimaryFromEnsData(topAddrs),
      fetchNamesFromNeynar(topAddrs),
      fetchUsersByFids(topFids)
    ]);

    function pickName(addr) {
      const k = String(addr || "").toLowerCase();
      const fidRaw = fidByAddr.get(k) || 0n;
      const fid = Number(fidRaw);

      // 1) Farcaster identity (preferred when available)
      if (Number.isFinite(fid) && fid > 0) {
        const uByFid = fidUserMap.get(fid);
        if (uByFid) return `${String(uByFid).replace(/^@/, "")}.farcaster.eth`;
      }
      ...
      // 4) Farcaster fallback by address (works only if address is verified/custody)
      const u = fcMap.get(k);
