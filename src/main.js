...
async function displayNameFor(addr) {
  // Avoid JSON.parse() on every row render (leaderboard renders up to 200 rows).
  // Keep a tiny in-memory cache of addrNameMap.
  if (!window.__addrNameMapCache) {
    try {
      const raw = localStorage.getItem("addrNameMap");
      window.__addrNameMapCache = raw ? JSON.parse(raw) : {};
    } catch {
      window.__addrNameMapCache = {};
    }
  }
  try {
    const m = window.__addrNameMapCache || {};
...
async function connectWallet() {
  ...
  try {
    ...
    if (typeof u === "string" && u && account) {
      ...
      try { window.__addrNameMapCache = m; } catch {}
    }
  } catch {}
  renderStatus();
  return account;
