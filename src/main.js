...
import { createBaseAccountSDK } from "https://esm.sh/@base-org/account@2.4.0";
...
const APP_ICON_URL = `${location.origin}/assets/icon-512.png`;
...
function appendDataSuffix(hexData) {
  // If a wallet doesn't support wallet_sendCalls { capabilities: { dataSuffix } },
  // we manually append the ERC-8021 attribution suffix to calldata so Base analytics can attribute it.
  if (!hexData || typeof hexData !== "string") return hexData;
  const d = hexData.startsWith("0x") ? hexData : `0x${hexData}`;
  const s = typeof dataSuffix === "string" ? dataSuffix : null;
  if (!s || !s.startsWith("0x")) return d;
  // Avoid double-append
  if (d.toLowerCase().endsWith(s.slice(2).toLowerCase())) return d;
  return `0x${d.slice(2)}${s.slice(2)}`;
}
...
(async () => {
  try {
    ...
    // Attempt a silent auto-connect so the app shows the correct address/name without extra taps.
    await connectWallet({ silent: true });
    await connectBaseAccount({ silent: true });
...
// =====================================================
// Base Account (Smart Account)
// =====================================================
// NOTE: Base.dev Builder Code analytics currently only attributes Smart Account txs.
// In Farcaster hosts, the injected wallet is usually an EOA, so txs won't show up on Base.dev unless we send via Base Account.
let baseAccountSdk = null;
let baseProvider = null;
let baseAccount = null;

function initBaseAccountSdk() {
  if (baseAccountSdk) return baseAccountSdk;
  try {
    ...
    console.warn("Base Account SDK init failed", e);
  }
  return baseAccountSdk;
}

async function connectBaseAccount({ silent = false } = {}) {
  initBaseAccountSdk();
  if (!baseProvider) return null;
  try {
    const method = silent ? "eth_accounts" : "eth_requestAccounts";
    const accts = await baseProvider.request({ method });
    if (Array.isArray(accts) && accts[0]) {
      baseAccount = accts[0];
      return baseAccount;
    }
  } catch (e) {
    if (!silent) console.warn("Base Account connect failed", e);
  }
  return null;
}

function getTxProvider() {
  return baseProvider || ethProvider;
}
...
async function getFcUsername({ retries = 5, delayMs = 250 } = {}) {
  // Tries to read Farcaster context user info. In some hosts, context becomes available
  // a moment after sdk.actions.ready(), so we retry briefly instead of returning null.
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      const u = ctx?.user || ctx?.context?.user || ctx?.context || null;

      const name = _toStr(u?.username ?? u?.displayName ?? u?.name ?? null);
      const fid = u?.fid ?? ctx?.fid ?? ctx?.userFid ?? null;
      ...
      // If user explicitly lacks username, don't overwrite an existing cached username.
      if (i === retries) break;
    } catch {
      // ignore and retry
    }
  ...
  // keep previous cached value if it existed
  return fcUsername || null;
}
...
function renderStatus() {
  ...
  const tag = baseAccount ? "Base Account" : "Base";

  try {
    const raw = localStorage.getItem("addrNameMap");
    const m = raw ? JSON.parse(raw) : {};
    const name = m?.[String(addr).toLowerCase()] || null;
    els.statusBadge.textContent = `${name || shortAddr(addr)} (${tag})`;
  } catch {
    els.statusBadge.textContent = `${shortAddr(addr)} (${tag})`;
  }
}
...
async function connectWallet({ silent = false } = {}) {
  ...
  if (!p) {
    if (!silent) toast("Open inside Farcaster/Base Mini App to connect wallet.");
    return null;
  }

  // Some hosts already have an account approved; avoid forcing a prompt when possible.
  let accs = [];
  try {
    accs = (await p.request({ method: "eth_accounts", params: [] })) || [];
  } catch {}

  if (!accs.length) {
    // In some hosts (esp. Farcaster), calling eth_requestAccounts during an
    // automatic/silent flow can trigger an external login redirect.
    // So for silent attempts we only read existing approvals.
    if (silent) return null;
    try {
      accs = (await p.request({ method: "eth_requestAccounts", params: [] })) || [];
    } catch (e) {
      if (!silent) toast("Wallet connect failed. Try again.");
      return null;
    }
  }
  ...
  // Cache Farcaster username for this address (if available)
  try {
    const u = await getFcUsername();
  ...
  // Also try to pick up an already-approved Base Account connection (no prompts).
  await connectBaseAccount({ silent: true });
...
async function ensureBase(p = null) {
  const provider = p || getTxProvider() || (await getProvider());
...
async function commitWeeklyOnchain() {
  ...
  // Host wallet (Farcaster/Base host) is used for Farcaster context + optional EOA interactions.
  if (!account) {
    await connectWallet({ silent: false });
  }

  // IMPORTANT: Base.dev Builder Code analytics currently only counts Smart Account transactions.
  // So we must send via Base Account (AA) when you want Base.dev to count it.
  if (!baseAccount) {
    await connectBaseAccount({ silent: false });
    if (!baseAccount) {
      toast("Connect Base Account first (needed for Base.dev Builder Code count).", 2500);
      return;
    }
  }
  ...
  try {
    const p = getTxProvider();
    if (!p) throw new Error("No tx provider");
    ...
    // Include Farcaster fid in the event payload (if available) so the leaderboard can
    // display Farcaster usernames even when the tx sender is a Smart Account.
    const fid = await getFcFid();

    let payload;
    if (fid !== null && fid !== undefined && fid !== "") {
      payload = encodeAbiParameters(
        [{ type: "uint256" }, { type: "uint256" }, { type: "uint256" }],
        [BigInt(pts), BigInt(weekStart), BigInt(fid)]
      );
    } else {
      payload = encodeAbiParameters(
    ...
    // Ensure ERC-8021 suffix is always present in calldata
    const callData = appendDataSuffix(data);
    ...
    const params = {
      ...
      chainId,
      atomicRequired: true,
      calls: [{ to: CONTRACT, value: "0x0", data: callData }],
      capabilities: { dataSuffix }
    };

    try {
      const callId = await p.request({ method: "wallet_sendCalls", params: [params] });

      // Best-effort: fetch call status (some wallets return tx hashes here).
      try {
        const st = await p.request({ method: "wallet_getCallsStatus", params: [callId] });
        const txHash = st?.receipts?.[0]?.transactionHash || st?.transactionHash || null;
        if (txHash) localStorage.setItem("w3r_last_tx", String(txHash));
      } catch {}
    } catch (e) {
      ...
      // Fallback: some providers still support eth_sendTransaction
      await p.request({
        method: "eth_sendTransaction",
        params: [{ from: baseAccount, to: CONTRACT, value: "0x0", data: callData }]
      });
    }
...
function openMainMenu() {
  const hostWalletLine = account ? shortAddr(account) : "Not connected";
  const baseWalletLine = baseAccount ? shortAddr(baseAccount) : "Not connected";
  ...
  openSheet(
    ...
    `
    <div class="menuGrid">
      <div class="kv"><div class="k">Host wallet</div><div class="v">${hostWalletLine}</div></div>
      <div class="kv"><div class="k">Base Account (AA)</div><div class="v">${baseWalletLine}</div></div>
    ...
    <div class="btnRow">
      <button class="pill" id="btnConnectHost">${account ? "Reconnect host wallet" : "Connect host wallet"}</button>
      <button class="pill" id="btnConnectBase">${baseAccount ? "Reconnect Base Account" : "Connect Base Account"}</button>
    </div>
  ...
  $("#btnConnectBase").addEventListener("click", async () => {
    const addr = await connectBaseAccount();
    if (!addr) toast("Base Account connection cancelled.");
    renderStatus();
    openMainMenu();
  });
...
async function openLeaderboardsView() {
  ...
  try {
    ...
    const youAddr = getActiveAddress();
    const weeklyIndex = youAddr
      ? weeklySorted.findIndex((x) => x.addr.toLowerCase() === youAddr.toLowerCase())
      : -1;
        ...
        <div class="subcopy">
          ${
            youAddr
              ...
              : "Connect Base Account to see your rank."
          }
        </div>
      </div>
