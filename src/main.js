...
function prefetchWeb3Deps() {
  const start = () => {
    ...
    ensureAttribution().catch(() => {});
    ensureViem().catch(() => {});
  };
  requestAnimationFrame(() => setTimeout(start, 350));
}
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    // Ensure web3 encoders + builder dataSuffix are available (loaded in background when possible)
    await Promise.all([ensureViem(), ensureAttribution()]);
