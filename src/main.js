...
// SFX + Haptics
// =====================================================
// NOTE: Mobile browsers often require a user gesture before audio/vibration works.
// We "unlock" both on first button press.
const COIN_SFX_URL = "/assets/coin.mp3";
const coinSfx = new Audio(COIN_SFX_URL);
coinSfx.preload = "auto";
coinSfx.volume = 0.65;

let _didUserGesture = false;
function markUserGesture() {
  if (_didUserGesture) return;
  _didUserGesture = true;
  // Best-effort: prime audio decode.
  try {
    coinSfx.load();
  } catch {}
}
...
function playCoinSfx() {
  if (!_didUserGesture) return;
  // Allow rapid re-trigger: if currently playing, clone for instant overlap.
  try {
    if (!coinSfx.paused && coinSfx.currentTime > 0 && coinSfx.currentTime < coinSfx.duration) {
      const a = coinSfx.cloneNode();
      a.volume = coinSfx.volume;
      a.play().catch(() => {});
    } else {
...
function hapticLight() {
  ...
  // Light, non-annoying tap feedback.
  navigator.vibrate(12);
}

function hapticCrash() {
  if (!_didUserGesture) return;
  if (!canVibrate()) return;
...
// =====================================================
function bindInstantTap(btn, fn, opts = {}) {
  const { haptic = false } = opts || {};
  btn.style.touchAction = "none";

  const onPointerDown = (ev) => {
    ev.preventDefault();
    ev.stopPropagation();
    // Unlock audio/vibration on first user interaction.
    markUserGesture();
...
bindInstantTap(els.leftBtn, () => moveLane(-1), { haptic: true });
bindInstantTap(els.rightBtn, () => moveLane(+1), { haptic: true });
