    ...
    const renderList = async (items) => {
      const rows = await Promise.all(
        items.map(async (x, i) => {
          ...
          // Full addresses make rows very tall on mobile. Show a compact form, but keep the
          // full address in a tooltip for power users.
          const name = isFullAddr ? shortAddr(nameStr) : nameStr;
          const titleAttr = isFullAddr ? ` title="${nameStr}"` : "";
          return `
            <div class="entry">
              <div class="left">
                <div class="rankBadge">#${i + 1}</div>
                <div class="addr${isFullAddr ? " addrFull" : ""}"${titleAttr}>${name}</div>
              </div>
