...
async function initWallet() {
  // try to read already-connected account without prompting
  try {
...
async function renderStatus() {
  ...
  const fcName = fcUser?.username ? `@${fcUser.username}` : null;

  // if no wallet connected yet
  if (!account) {
    // still show farcaster name if present, but keep affordance to connect
    els.statusBadge.textContent = fcName ? `${fcName} · Connect` : "Connect";
    return;
  }

  // wallet connected
  const walletLabel = `${shortAddr(account)} (Base)`;
  els.statusBadge.textContent = fcName ? `${fcName} · ${walletLabel}` : walletLabel;
}
...
els.statusBadge.addEventListener("click", async () => {
  // If badge shows Connect or no account, (re)connect.
  const needsConnect = !account || /Connect/i.test(els.statusBadge.textContent || "");
  if (needsConnect) {
    await connectWallet();
  } else {
    // open menu when already connected
    openMainMenu();
  }
  await renderStatus();
});
