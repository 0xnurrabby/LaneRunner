...
function isMagnetOn(now = Date.now()) {
  return game.magnetUntil && now < game.magnetUntil;
}
function isSlowOn(now = Date.now()) {
  return game.slowUntil && now < game.slowUntil;
}
function isShieldOn(now = Date.now()) {
  return game.shieldUntil && now < game.shieldUntil;
}
function isDoubleLaneOn(now = Date.now()) {
  return game.dblUntil && now < game.dblUntil;
}

function powerupCountdownText(now = Date.now()) {
  const parts = [];
