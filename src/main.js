...
function normalizeCastText(text, maxLen = 320) {
...
function openComposerImmediate(text, embeds = [HOME_URL]) {
  ...
  const safeText = normalizeCastText(text, 320);
  const embedArr = Array.isArray(embeds) ? embeds.filter(Boolean).map((e) => String(e)) : [String(embeds)];

  // Prefer native composeCast (Mini App SDK).
  if (s?.actions?.composeCast) {
    try {
      return Promise.resolve(
        s.actions.composeCast({
          text: safeText,
          embeds: embedArr.slice(0, 2) // SDK supports max 2 embeds
        })
      )
        .then((result) => ({ opened: true, posted: !!result?.cast }))
        .catch(() => ({ opened: false, posted: false }));
    } catch {
      // fall through
    }
  }

  // Fallback: open Warpcast compose URL through host.
  const url = buildWarpcastComposeUrl(safeText, embedArr.slice(0, 2));

  if (s?.actions?.openUrl) {
    try {
      return Promise.resolve(s.actions.openUrl({ url }))
        .then(() => ({ opened: true, posted: null }))
        .catch(() => ({ opened: false, posted: false }));
    } catch {}
    try {
      return Promise.resolve(s.actions.openUrl(url))
  ...
  try {
    ...
    return Promise.resolve({ opened: true, posted: null });
  } catch {
    return Promise.resolve({ opened: false, posted: false });
  }
}
...
function shareYourStatsFromGesture() {
  try {
    ...
    // Don't `await` before triggering share — user-gesture gated in many clients.
    if (!account) {
    ...
    const { weeklySorted, prevWeekSorted, allTimeSorted } = lastLeaderboardData;
    ...
    const fmtLine = (label, r) => {
      ...
      return `${label}: ${rankStr} • ${ptsStr}`;
    };

    // Keep suggested cast text short (casts have strict length limits).
    const text = [
      `🏃 Web3 Runner — ${who}`,
      fmtLine('All-time', a),
      fmtLine('This week', w),
    ...
    pOpen.then((res) => {
      if (!res?.opened) {
        toast('Share failed to open. Please try again.');
        return;
      }
      if (res?.posted === false) {
      ...
      renderHud();
      updateShareCounterUI();
      toast(`Shared! +${SHARE_REWARD_POINTS} points (${used + 1}/${SHARE_DAILY_LIMIT} today)`);
    });
  } catch (e) {
