...
function _getSdkSync() {
  // Different hosts expose the SDK on different globals.
  // - Base app: window.miniapp.sdk
  // - Some Farcaster clients / older SDK builds: window.frame.sdk
  // - Some Farcaster clients / UMD builds: window.sdk
  return (
...
// Farcaster Mini App SDK (official package). We load it lazily so the game renders instantly.
// This is the SDK described in the Farcaster docs (import { sdk } from '@farcaster/miniapp-sdk').
let fcMiniSdk = null;
let _fcMiniSdkPromise = null;

function prefetchFarcasterMiniSdk() {
  if (fcMiniSdk) return Promise.resolve(fcMiniSdk);
  _fcMiniSdkPromise = _fcMiniSdkPromise ||
    import("https://esm.sh/@farcaster/miniapp-sdk@0.2.1")
      .then((m) => {
        fcMiniSdk = m?.sdk || null;
        return fcMiniSdk;
      })
      .catch(() => null);
  return _fcMiniSdkPromise;
}
...
(async () => {
  try {
    ...
    // Fallback: try the official Farcaster miniapp SDK if the host global isn't available.
    try {
      const fc = await prefetchFarcasterMiniSdk();
      if (fc?.actions?.ready) {
        await fc.actions.ready({ disableNativeGestures: true });
      }
    } catch {
...
prefetchWeb3Deps();
// Also prefetch the Farcaster Mini App SDK (official package) so Share can fire without awaiting imports.
prefetchFarcasterMiniSdk();
...
async function displayNameFor(addr) {
  ...
  // If no username/ENS/Basename is available, show the FULL address (not truncated).
  // (We keep shortAddr() for other UI areas like the top badge.)
...
// Cache the last-loaded leaderboard data so the Share button can use it.
let lastLeaderboardData = null;

const SHARE_DAILY_LIMIT = 10;
const SHARE_REWARD_POINTS = 1000;
...
function shareCountStorageKey(userId) {
  return `w3r_share_count_${String(userId || "anon")}_${utcDayKey()}`;
}

function getDailyShareCount(userId) {
  try {
    return Number(localStorage.getItem(shareCountStorageKey(userId)) || "0") || 0;
  } catch {
    return 0;
  }
}

function setDailyShareCount(userId, n) {
  try {
    localStorage.setItem(shareCountStorageKey(userId), String(Math.max(0, Math.floor(n || 0))));
  } catch {}
}

async function getShareUserId() {
  // Prefer Farcaster fid if available so the counter is stable even before wallet connect.
  const fid = await getFcFid().catch(() => null);
  if (fid) return String(fid);
  if (account) return String(account);
  return "anon";
}

async function updateShareCounterUI() {
  const el = document.getElementById("shareCounter");
  if (!el) return;
  const userId = account ? String(account) : (fcFid ? String(fcFid) : "anon");
  const used = getDailyShareCount(userId);
  el.innerHTML = `Share <span class="shareUsed">${used}</span>/<span class="shareLimit">${SHARE_DAILY_LIMIT}</span>`;

  const btn = document.getElementById("shareStatsBtn");
  if (btn) {
    // Never disable the button just because leaderboards/wallet aren't ready.
    // A disabled button feels "dead" in mobile webviews.
    // We only disable if the user hits the daily limit.
    btn.disabled = used >= SHARE_DAILY_LIMIT;
  }
}

function pleasantColorForIndex(i) {
  // Golden-angle distribution for nice variety with minimal clumping.
  const hue = (i * 137.508) % 360;
  // Slightly muted so it doesn't look neon.
  return `hsla(${hue.toFixed(3)}, 52%, 58%, 0.95)`;
}

function pleasantRowBgForIndex(i) {
  // Same golden-angle palette, but softened for row backgrounds.
  const hue = (i * 137.508) % 360;
  return `hsla(${hue.toFixed(3)}, 55%, 60%, 0.16)`;
}

function pleasantRowBorderForIndex(i) {
  const hue = (i * 137.508) % 360;
  return `hsla(${hue.toFixed(3)}, 60%, 62%, 0.28)`;
}

function findRankAndPts(list, addr) {
  if (!addr) return { rank: null, pts: 0n };
  const idx = list.findIndex((x) => String(x.addr || "").toLowerCase() === String(addr).toLowerCase());
  if (idx < 0) return { rank: null, pts: 0n };
  return { rank: idx + 1, pts: list[idx]?.pts ?? 0n };
}

function buildWarpcastComposeUrl(text, embeds = []) {
  let url = `https://warpcast.com/~/compose?text=${encodeURIComponent(String(text || ""))}`;
  for (const e of embeds || []) {
    if (!e) continue;
    url += `&embeds[]=${encodeURIComponent(String(e))}`;
  }
  return url;
}

// NOTE: Farcaster/Warpcast treats some SDK actions as *user-gesture gated*.
// So we must trigger the share action immediately from the pointer event handler
// (no awaits beforehand), otherwise the host may ignore the request.
function getShareUserIdSync() {
  if (fcFid) return String(fcFid);
  if (account) return String(account);
  return "anon";
}

function openComposerImmediate(text, embeds = [HOME_URL]) {
  // IMPORTANT: Farcaster Mini Apps expects the official SDK package (`@farcaster/miniapp-sdk`).
  // We prefetch it in the background (see prefetchFarcasterMiniSdk()). If present, use it first.
  const fc = fcMiniSdk;
  const host = _getSdkSync() || sdk;

  const safeText = String(text || "");
  const embedArrRaw = Array.isArray(embeds) ? embeds.filter(Boolean).map((e) => String(e)) : [String(embeds)];
  const embedArr = embedArrRaw.slice(0, 2); // Farcaster max 2 embeds

  // 1) Official Farcaster miniapp SDK
  if (fc?.actions?.composeCast) {
    try {
      return Promise.resolve(fc.actions.composeCast({ text: safeText, embeds: embedArr }))
        .then((res) => {
          // If the user cancels, res.cast can be null. Treat that as "not shared".
          return !!(res && res.cast);
        })
        .catch(() => false);
    } catch {
  ...
  // 2) Host-provided SDKs (Base / older Farcaster builds)
  if (host?.actions?.composeCast) {
    try {
      return Promise.resolve(host.actions.composeCast({ text: safeText, embeds: embedArr }))
        .then((res) => {
          return !!(res && res.cast);
        })
        .catch(() => false);
  ...
  // Fallback: open Warpcast compose URL through host (still gesture-gated in many clients).
  const url = buildWarpcastComposeUrl(safeText, embedArr);

  if (host?.actions?.openUrl) {
    try {
      return Promise.resolve(host.actions.openUrl(url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
  ...
  if (host?.actions?.openLink) {
    try {
      return Promise.resolve(host.actions.openLink(url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
...
function shareYourStatsFromGesture() {
  try {
    if (!lastLeaderboardData) {
      toast("Leaderboard not loaded yet. Tap Refresh.");
      return;
    }

    // Important: don't `await connectWallet()` here — it breaks the user-gesture chain in Farcaster.
    if (!account) {
      toast("Connect wallet first to share your stats.");
      return;
    }

    const userId = getShareUserIdSync();
    const used = getDailyShareCount(userId);
    if (used >= SHARE_DAILY_LIMIT) {
      toast(`Daily share limit reached (${SHARE_DAILY_LIMIT}/${SHARE_DAILY_LIMIT}).`);
      updateShareCounterUI();
      return;
    }

    const { weeklySorted, prevWeekSorted, allTimeSorted, weekStart, prevWeekStart } = lastLeaderboardData;
    const w = findRankAndPts(weeklySorted, account);
    const p = findRankAndPts(prevWeekSorted, account);
    const a = findRankAndPts(allTimeSorted, account);

    const who = fcUsername ? `@${fcUsername}` : shortAddr(account);
    const weekLabel = new Date(weekStart).toISOString().slice(0, 10);
    const prevLabel = new Date(prevWeekStart).toISOString().slice(0, 10);

    const fmtLine = (label, r) => {
      const rankStr = r.rank ? `#${r.rank}` : "—";
      const ptsStr = fmtPts(r.pts || 0n);
      return `${label}: ${rankStr} • ${ptsStr} pts`;
    };

    const text = [
      `🏃 Web3 Runner stats — ${who}`,
      "",
      fmtLine("All-time", a),
      fmtLine(`This week (since ${weekLabel} UTC)`, w),
      fmtLine(`Last week (since ${prevLabel} UTC)`, p),
      "",
      "Play the mini app on Farcaster/Base:",
      HOME_URL
    ].join("\n");

    // Trigger the host action immediately (no awaits before this line).
    const pOpen = openComposerImmediate(text, [HOME_URL]);

    pOpen.then((opened) => {
      if (!opened) {
        toast("Share failed to open in Farcaster. Please try again.");
        return;
      }
      // Reward + limit
      setDailyShareCount(userId, used + 1);
      profile.bankPoints += SHARE_REWARD_POINTS;
      persistProfile();
      renderHud();
      updateShareCounterUI();
      toast(`Share opened! +${SHARE_REWARD_POINTS} points (${used + 1}/${SHARE_DAILY_LIMIT} today)`);
    });
  } catch (e) {
...
async function openLeaderboardsView() {
  ...
  openSheet(
    ...
    <div class="btnRow" style="margin-top:12px">
      <button class="pill" id="backMenu">Back</button>
      <div class="pillStack" id="sharePillStack">
        <button class="pill pillShare" id="shareStatsBtn">Share your stat</button>
        <div class="shareCounter" id="shareCounter">Share <span class="shareUsed">0</span>/<span class="shareLimit">${SHARE_DAILY_LIMIT}</span></div>
      </div>
  ...
  const _shareBtn = $("#shareStatsBtn");
  _shareBtn.addEventListener(
    "pointerdown",
    (e) => {
      e.preventDefault();
      e.stopPropagation();
      shareYourStatsFromGesture();
    },
    { passive: false }
  );
  ...
  // Update immediately (before wallet connect) so user sees today's share progress.
  updateShareCounterUI();

  try {
    const data = await fetchLeaderboard();
    lastLeaderboardData = data;
    // Re-check share limit now that leaderboard is loaded.
    updateShareCounterUI();
    ...
    const renderList = async (items, opts = {}) => {
      ...
      const entryClass = String(opts.entryClass || "").trim();
      const rows = await Promise.all(
        items.map(async (x, i) => {
          const nameRaw = (typeof x.name === "string" && x.name) ? x.name : await displayNameFor(x.addr);
          const nameStr = String(nameRaw || "");
          const isFullAddr = /^0x[a-fA-F0-9]{40}$/.test(nameStr);
          // Default: keep rows compact. For Last week, show full address as requested.
          let name = nameStr;
          let titleAttr = "";
          let addrClass = "addr";
          if (isFullAddr) {
            addrClass += " addrFull";
            if (showFullAddr) {
              addrClass += " addrShowAll";
              name = nameStr; // full
            } else {
              name = shortAddr(nameStr);
              titleAttr = ` title="${nameStr}"`;
            }
          }

          const entryCls = `entry${entryClass ? " " + entryClass : ""}`;
          const rowStyle = colorize
            ? ` style="--rowbg:${pleasantRowBgForIndex(i)}; --rowbd:${pleasantRowBorderForIndex(i)}"`
            : "";
          return `
            <div class="${entryCls}"${rowStyle}>
              <div class="left">
                <div class="rankBadge">#${i + 1}</div>
                <div class="${addrClass}"${titleAttr}>${name}</div>
              </div>
    ...
    const winnersHtml =
      lastWinners.length === 0
        ...
        : `<div class="boardList">${await renderList(lastWinners, { showFullAddr: true, colorize: true, entryClass: "lastWeekEntry" })}</div>`;
