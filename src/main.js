...
// =====================================================
// Ethers.js helper (for Web3RunnerLogger.web3RunnerInteract)
// =====================================================
let _ethersPromise = null;
async function ensureEthers() {
  _ethersPromise = _ethersPromise || import("https://esm.sh/ethers@6.13.5?bundle");
  return _ethersPromise;
}

async function logInteractionWithEthers(actionText = "RUN", dataHex = "0x") {
  if (!LOGGER_CONTRACT || LOGGER_CONTRACT === "0x0000000000000000000000000000000000000000") {
    toast("Set LOGGER_CONTRACT in src/main.js first.");
    return;
  }
  ...
  try {
    ...
    const { BrowserProvider, Contract, ethers } = await ensureEthers();
    ...
    const contract = new Contract(LOGGER_CONTRACT, abi, signer);
    const action = ethers.encodeBytes32String(String(actionText || "RUN"));

    toast("Confirm in wallet…", 1400);
    const tx = await contract.web3RunnerInteract(action, String(dataHex || "0x"));
    toast("Sent! Waiting…", 1400);
    await tx.wait();
    toast("Logged! Check BaseScan event logs.");
  } catch (e) {
    toast(e?.message ? String(e.message) : "Logger call failed");
  }
}
...
// -----------------------------------------------------
// On-chain interaction logger (BaseScan Event Logs)
// Your deployed contract from the screenshot:
//   function web3RunnerInteract(bytes32 action, bytes data) external
//
// ✅ Set this to your deployed Web3RunnerLogger contract address:
const LOGGER_CONTRACT = "0x0000000000000000000000000000000000000000";
...
function openMainMenu() {
  ...
  openSheet(
    ...
    <div class="btnRow">
      <button class="pill" id="btnLog">Log interaction (BaseScan)</button>
    </div>
  ...
  $("#btnLog").addEventListener("click", () => logInteractionWithEthers("RUN", "0x"));
...
// -----------------------------------------------------
// Palette helpers (used for Last week row backgrounds)
// -----------------------------------------------------
function pleasantRowBgForIndex(i) {
  // Golden-angle distribution for variety; softened for backgrounds.
