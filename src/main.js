...
async function updateShareCounterUI() {
  ...
  if (btn) {
    // Never block clicks while leaderboards are loading — otherwise the button feels "dead".
    // Only disable when the daily limit is reached.
...
function openComposerImmediate(text, embeds = [HOME_URL]) {
  ...
  // Some hosts expose actions under `actions`, others may expose methods at the root.
  const composeCast = (s && s.actions && s.actions.composeCast) || (s && s.composeCast);
  if (composeCast) {
    try {
      return Promise.resolve(composeCast.call(s.actions || s, { text: safeText, embeds: embedArr }))
        .then(() => true)
        .catch(() => false);
    } catch {
  ...
  const openUrl = (s && s.actions && s.actions.openUrl) || (s && s.openUrl);
  if (openUrl) {
    try {
      return Promise.resolve(openUrl.call(s.actions || s, url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
  ...
  const openLink = (s && s.actions && s.actions.openLink) || (s && s.openLink);
  if (openLink) {
    try {
      return Promise.resolve(openLink.call(s.actions || s, url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
  ...
  // Base and some webviews support the native share sheet.
  // It's not a Farcaster composer, but it's better than a dead click.
  if (navigator && typeof navigator.share === "function") {
    try {
      const shareUrl = embedArr && embedArr[0] ? String(embedArr[0]) : HOME_URL;
      // Do not await — keep it in the gesture chain.
      navigator.share({ text: safeText, url: shareUrl });
      return Promise.resolve(true);
    } catch {}
  }

  // As a last resort, try opening a new tab (some hosts allow this).
  try {
