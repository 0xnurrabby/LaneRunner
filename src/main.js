...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    persistProfile();
    toast("Committed on-chain! Updating leaderboard… (Base.dev attribution may lag in some Farcaster clients)", 2800);
...
function openMainMenu() {
  ...
  openSheet(
    ...
    <div class="fineprint">
      ...
      <span style="opacity:.8">Note: Base.dev Builder Code analytics may not update for transactions sent from some Farcaster clients yet (EOA support is rolling out). Base App attribution works immediately.</span>
    </div>
  `
  );
