...
const BUILDER_CODE_UI = "bc_nkm4ufdx";
// Base.dev gives the raw builder code as a short random string (e.g. "abc123").
// The UI may display it with a "bc_" prefix—strip that before generating the ERC-8021 suffix.
const BUILDER_CODE = BUILDER_CODE_UI.startsWith("bc_") ? BUILDER_CODE_UI.slice(3) : BUILDER_CODE_UI;
...
// Note: Base.dev builder-code analytics currently count Smart Account (AA) transactions.
// If a host wallet sends an EOA transaction, the attribution suffix is still appended/preserved,
// but Base.dev may not show the count until EOA support is enabled.
...
function renderStatus() {
  ...
  try {
    ...
    els.statusBadge.textContent = `${name || shortAddr(account)} (Base)`;
  } catch {
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    // Always include ERC-8021 suffix in calldata so attribution survives even if
    // the wallet ignores `capabilities.dataSuffix`.
    ...
    try {
      ...
      try {
        ...
        if (txHash) {
          localStorage.setItem("w3r_last_tx", String(txHash));
        }
      } catch {}
    } catch (e) {
      ...
      await p.request({
        ...
        params: [{ from: account, to: CONTRACT, value: "0x0", data: callData }]
      });
    }
...
function openMainMenu() {
  ...
  openSheet(
    ...
    <div class="fineprint">
      ...
      Builder code (UI): ${BUILDER_CODE_UI}<br/>Builder code (raw): ${BUILDER_CODE}
    </div>
  `
  );
