...
async function displayNameFor(addr) {
  ...
  // Prefer Farcaster identity (FID) when available (independent of wallet address linkage)
  if (account && addr) {
...
async function renderStatus() {
  ...
  try {
    const name = await displayNameFor(account);
    els.statusBadge.textContent = `${name} (Base)`;
  } catch {
...
els.statusBadge.addEventListener("click", async () => {
  const txt = (els.statusBadge.textContent || "").trim();
  // If not connected (or UI says Connect), trigger wallet connect
  if (!account || txt === "Connect") {
    await connectWallet();
    await renderStatus();
    return;
  }
...
async function openMainMenu() {
  const walletLine = account ? await displayNameFor(account) : "Not connected";
