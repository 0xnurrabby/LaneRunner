...
function drawCarTopDown(x, y, w, h, bodyColor) {
  // New illustrated top-down car (reference-style). Rendering only — no gameplay logic touched.
  ctx.save();

  // --- helpers ---
  function clamp(n, a, b) { return Math.max(a, Math.min(b, n)); }
  function parseColor(c) {
    if (typeof c !== "string") return { r: 220, g: 60, b: 70 };
    let s = c.trim();
  ...
  // Make sure the car always looks good in small sizes
  const lw = clamp(w * 0.065, 1.2, 5.5);
  const rr = clamp(w * 0.33, 6, 22);

  // Outer bounds (slight inset so it "fits" lanes nicely)
  const bx = x + w * 0.06;
  const by = y + h * 0.03;
  const bw = w * 0.88;
  const bh = h * 0.94;

  const base = parseColor(bodyColor);
  ...
  ctx.beginPath();
  ctx.ellipse(x + w * 0.52, y + h * 0.54, bw * 0.48, bh * 0.46, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  // --- body fill (center brighter, edges darker) ---
  const g = ctx.createLinearGradient(bx, 0, bx + bw, 0);
  g.addColorStop(0, rgba(edge, 1));
  g.addColorStop(0.22, rgba(dark, 1));
  g.addColorStop(0.50, rgba(mix(base, { r: 255, g: 255, b: 255 }, 0.10), 1));
  g.addColorStop(0.78, rgba(dark, 1));
  g.addColorStop(1, rgba(edge, 1));
  ctx.fillStyle = g;
  ...
  // --- inner inset (gives that illustrated border depth) ---
  ctx.globalAlpha = 0.35;
  ctx.lineWidth = clamp(lw * 0.55, 1, 4);
  ctx.strokeStyle = "rgba(255,255,255,0.35)";
  drawRoundedRect(bx + bw * 0.02, by + bh * 0.02, bw * 0.96, bh * 0.96, clamp(rr * 0.85, 5, 18));
  ctx.stroke();
  ctx.globalAlpha = 1;
  ...
  const wg = ctx.createLinearGradient(wx, wy, wx + ww * 0.7, wy + wh);
  ...
  // --- side dark panels (like the reference) ---
  ...
  drawRoundedRect(bx + bw - sx - sw, sy, sw, sh, clamp(sw * 0.55, 6, 18));
  ctx.fill();
  ctx.globalAlpha = 1;

  // --- subtle hood highlights (no center stripe) ---
  ctx.globalAlpha = 0.18;
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  drawRoundedRect(bx + bw * 0.18, by + bh * 0.10, bw * 0.10, bh * 0.26, clamp(bw * 0.12, 6, 16));
  drawRoundedRect(bx + bw * 0.72, by + bh * 0.12, bw * 0.07, bh * 0.22, clamp(bw * 0.10, 6, 14));
  ctx.fill();
  ctx.globalAlpha = 1;
  ...
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  ctx.beginPath();
  ctx.arc(bx + bw * 0.24, by + bh * 0.92, Math.max(1.6, w * 0.045), 0, Math.PI * 2);
  ctx.arc(bx + bw * 0.76, by + bh * 0.92, Math.max(1.6, w * 0.045), 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
...
function drawPlayerCarPremium(x, y, w, h) {
  // Player car: same reference-style body, plus a soft premium glow (render-only).
  ctx.save();

  // Glow (kept subtle so it doesn't look "boxy")
  ctx.globalAlpha = 0.55;
  ctx.shadowColor = "rgba(120,210,255,0.55)";
  ctx.shadowBlur = Math.max(6, w * 0.22);
  ctx.shadowOffsetX = 0;
  ctx.shadowOffsetY = 0;

  // Use the same renderer. (No stripe / no logic changes)
  drawCarTopDown(x, y, w, h, playerColor || "#ff3b5c");
