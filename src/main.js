...
// Optional fallback: use Base Account SDK outside the Base App.
// Base.dev builder-code analytics currently only shows attribution for Smart Account (AA)
// transactions, so this helps Farcaster clients that otherwise send EOA txs.
import {
  createBaseAccountSDK,
  base as baseAccountChains
} from "https://esm.sh/@base-org/account@latest";
import {
  createPublicClient,
  decodeEventLog,
  decodeAbiParameters,
...
let ethProviderKind = null; // 'miniapp' | 'baseaccount'
let baseAccountProvider = null;

async function getBaseAccountProvider() {
  if (baseAccountProvider) return baseAccountProvider;
  // Use the current origin for the logo URL to avoid hardcoding a domain.
  const appLogoUrl = `${location.origin}/assets/icon-512.png`;
  const sdkBA = createBaseAccountSDK({
    appName: APP_NAME,
    appLogoUrl,
    appChainIds: [baseAccountChains.constants.CHAIN_IDS.base]
  });
  baseAccountProvider = sdkBA.getProvider();
  return baseAccountProvider;
}

async function isBaseAppClient() {
  try {
    const ctx = await sdk.context;
    return ctx?.client?.clientFid === 309857;
  } catch {
    return false;
  }
}
...
async function getProvider() {
  ...
  // IMPORTANT: Base.dev builder-code analytics currently only displays attribution for
  // Smart Account (AA) transactions. Many Farcaster clients expose an EOA provider.
  // Outside the Base App, prefer Base Account SDK to submit AA transactions.
  const inBaseApp = await isBaseAppClient();
  if (!inBaseApp) {
    try {
      ethProvider = await getBaseAccountProvider();
      ethProviderKind = "baseaccount";
      return ethProvider;
    } catch {
      // fall through
    }
  ...
  // Base App (or if Base Account init fails): use the embedded miniapp provider.
  try {
    ethProvider = await sdk.wallet.getEthereumProvider();
    ethProviderKind = "miniapp";
    return ethProvider;
  } catch {
    // Final fallback: Base Account SDK (for regular browsers)
    try {
      ethProvider = await getBaseAccountProvider();
      ethProviderKind = "baseaccount";
      return ethProvider;
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      const ctxRaw =
        ...
        null;
      const ctx =
        ctxRaw && typeof ctxRaw === "object" && typeof ctxRaw.then === "function"
          ? await ctxRaw
          : ctxRaw;
...
function renderStatus() {
  ...
  const modeLabel = ethProviderKind === "baseaccount" ? "Base Account" : "Base";
  // Prefer showing Farcaster username (if available) for the connected address
  try {
    const raw = localStorage.getItem("addrNameMap");
    const m = raw ? JSON.parse(raw) : {};
    const name = m?.[String(account).toLowerCase()] || null;
    els.statusBadge.textContent = `${name || shortAddr(account)} (${modeLabel})`;
  } catch {
    els.statusBadge.textContent = `${shortAddr(account)} (${modeLabel})`;
  }
}
  ...
  try {
    ...
    try {
      ...
      await p.request({
        ...
        params: [{ from: account, to: CONTRACT, value: "0x0", data: appendDataSuffix(data) }]
      });
    }
