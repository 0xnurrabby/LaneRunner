...
async function getProvider() {
  // Prefer an injected EIP-1193 provider (e.g., MetaMask / in-app wallet) when available.
...
async function getFcUsername() {
  ...
  try {
    ...
    fcUsername = u?.username || u?.displayName || u?.name || null;
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    toast("Confirm transaction in your wallet…", 2600);
    ...
    const params = {
      ...
      // Keep capabilities empty for maximum wallet compatibility (matches working miniapps)
      capabilities: {}
    };

    // Try EIP-5792 batch call first (shows wallet confirm in supported wallets)
    let submitted = false;
    try {
      await p.request({ method: "wallet_sendCalls", params: [params] });
      submitted = true;
      toast("Confirm in wallet…", 1800);
    } catch (e) {
      ...
      // Fallback: normal transaction (MetaMask-style)
      try {
        toast("Confirm in wallet…", 1800);
        await p.request({
  ...
  try {
    ...
    const renderList = async (items) => {
      const rows = await Promise.all(
        items.map(async (x, i) => {
          const name = await displayNameFor(x.addr);
          return `
            <div class="entry">
              <div class="left">
