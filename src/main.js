...
import { mainnet, base } from "https://esm.sh/viem/chains@2.21.0";
...
// Base docs describe the builder code as a random string like "k3p9da".
// In Base.dev UI it may be shown with a "bc_" prefix — we strip it for ERC-8021 encoding.
const BUILDER_CODE_RAW = BUILDER_CODE.startsWith("bc_") ? BUILDER_CODE.slice(3) : BUILDER_CODE;
const dataSuffix = Attribution.toDataSuffix({ codes: [BUILDER_CODE_RAW] });

// =====================================================
// Name Resolution (Basenames via ENSIP-19) + optional Farcaster mapping
// =====================================================

// Basenames are ENS-compatible names on Base (ENSIP-19). We resolve them via Ethereum mainnet
// Universal Resolver + the Base coinType.
// Uses a public mainnet RPC (Cloudflare) to avoid needing an API key.
...
// Simple in-memory cache to avoid spamming RPC
const nameCache = new Map();

async function resolveBasename(addr) {
  const k = String(addr || "").toLowerCase();
  if (!k || !k.startsWith("0x") || k.length !== 42) return null;
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    try {
      ...
      toast("Transaction sent (wallet_sendCalls). Base.dev analytics may take a bit to update.", 2200);
    } catch (e) {
      ...
      await p.request({
      toast("wallet_sendCalls failed; using eth_sendTransaction (may NOT count on Base.dev yet).", 2600);
