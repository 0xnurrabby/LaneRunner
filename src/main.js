...
function drawCarTopDown(x, y, w, h, bodyColor) {
  ...
  // Subtle shadow so the car feels "grounded"
  ctx.globalAlpha = 0.35;
  ctx.fillStyle = "rgba(0,0,0,0.45)";
  ctx.beginPath();
  ctx.ellipse(x + w / 2, y + h * 0.93, w * 0.46, h * 0.14, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  // Keep sprite bounds identical to gameplay collision bounds (UI-only redesign)
  const bx = x;
  const by = y;
  const bw = w;
  const bh = h;
  ...
  // Body
  const r = Math.min(18, bw * 0.30);
  ctx.fillStyle = bodyColor;
  drawRoundedRect(bx, by, bw, bh, r);
  ctx.fill();

  // Light outline for definition
  ctx.strokeStyle = "rgba(255,255,255,0.24)";
  ctx.lineWidth = Math.max(1.4, bw * 0.035);
  ctx.stroke();
  ...
  ctx.fillStyle = gloss;
  drawRoundedRect(bx + bw * 0.07, by + bh * 0.06, bw * 0.86, bh * 0.40, Math.min(14, bw * 0.28));
  ctx.fill();

  // Racing stripes (2 thin stripes + a faint center shadow)
  const stripeW = Math.max(2.2, bw * 0.07);
  const stripeGap = stripeW * 0.45;
  const stripeTop = by + bh * 0.06;
  const stripeH = bh * 0.88;
  const sx1 = bx + bw / 2 - stripeGap / 2 - stripeW;
  const sx2 = bx + bw / 2 + stripeGap / 2;

  ctx.fillStyle = "rgba(255,255,255,0.70)";
  drawRoundedRect(sx1, stripeTop, stripeW, stripeH, stripeW);
  drawRoundedRect(sx2, stripeTop, stripeW, stripeH, stripeW);
  ctx.fill();

  ctx.globalAlpha = 0.16;
  ctx.fillStyle = "rgba(0,0,0,1)";
  drawRoundedRect(bx + bw * 0.46, stripeTop, bw * 0.08, stripeH, bw * 0.06);
  ctx.fill();
  ctx.globalAlpha = 1;

  // Windshield / cabin (top)
  ctx.fillStyle = "rgba(0,0,0,0.50)";
  drawRoundedRect(bx + bw * 0.20, by + bh * 0.14, bw * 0.60, bh * 0.28, Math.min(12, bw * 0.26));
  ctx.fill();

  // Cabin reflection
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  drawRoundedRect(bx + bw * 0.26, by + bh * 0.18, bw * 0.48, bh * 0.10, Math.min(10, bw * 0.22));
  ctx.fill();

  // Rear window (bottom)
  ctx.fillStyle = "rgba(0,0,0,0.34)";
  drawRoundedRect(bx + bw * 0.24, by + bh * 0.56, bw * 0.52, bh * 0.20, Math.min(12, bw * 0.26));
  ctx.fill();

  // Wheels
  ctx.fillStyle = "rgba(0,0,0,0.62)";
  const ww = Math.max(5, bw * 0.16);
  const wh = Math.max(12, bh * 0.20);
  const wy1 = by + bh * 0.20;
  const wy2 = by + bh * 0.62;
  drawRoundedRect(bx - ww * 0.25, wy1, ww, wh, ww * 0.55);
  drawRoundedRect(bx + bw - ww * 0.75, wy1, ww, wh, ww * 0.55);
  drawRoundedRect(bx - ww * 0.25, wy2, ww, wh, ww * 0.55);
  drawRoundedRect(bx + bw - ww * 0.75, wy2, ww, wh, ww * 0.55);
  ctx.fill();

  // Mirrors (tiny)
  ctx.globalAlpha = 0.85;
  ctx.fillStyle = "rgba(255,255,255,0.18)";
  drawRoundedRect(bx - ww * 0.05, by + bh * 0.30, ww * 0.55, wh * 0.35, ww * 0.35);
  drawRoundedRect(bx + bw - ww * 0.50, by + bh * 0.30, ww * 0.55, wh * 0.35, ww * 0.35);
  ctx.fill();
  ctx.globalAlpha = 1;

  // Lights (headlights + tail lights)
  const lightR = Math.max(2.3, bw * 0.055);

  ctx.fillStyle = "rgba(245,252,255,0.80)";
  ctx.beginPath();
  ctx.arc(bx + bw * 0.26, by + bh * 0.10, lightR, 0, Math.PI * 2);
  ctx.arc(bx + bw * 0.74, by + bh * 0.10, lightR, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = "rgba(255,90,120,0.70)";
  ctx.beginPath();
  ctx.arc(bx + bw * 0.28, by + bh * 0.92, lightR * 0.85, 0, Math.PI * 2);
  ctx.arc(bx + bw * 0.72, by + bh * 0.92, lightR * 0.85, 0, Math.PI * 2);
  ctx.fill();
...
function drawPlayerCarPremium(x, y, w, h) {
  ...
  // Soft neon glow under the player car
  ctx.globalAlpha = 0.75;
  ctx.fillStyle = "rgba(90,190,255,0.28)";
  ctx.beginPath();
  ctx.ellipse(x + w / 2, y + h * 0.93, w * 0.55, h * 0.18, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;

  // Premium blue paint (gradient)
  const paint = ctx.createLinearGradient(x, y, x, y + h);
  paint.addColorStop(0, "rgba(120,230,255,0.98)");
  paint.addColorStop(0.55, "rgba(70,170,255,0.98)");
  paint.addColorStop(1, "rgba(35,95,220,0.98)");

  // Reuse the same top-down car shape (UI-only change, no gameplay logic touched)
  drawCarTopDown(x, y, w, h, paint);

  // Extra edge glow for the player car
  ctx.globalAlpha = 0.42;
  ctx.strokeStyle = "rgba(90,190,255,0.38)";
  ctx.lineWidth = Math.max(1.5, w * 0.04);
  drawRoundedRect(x + w * 0.02, y + h * 0.02, w * 0.96, h * 0.96, Math.min(20, w * 0.32));
  ctx.stroke();
  ctx.globalAlpha = 1;
