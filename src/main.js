...
async function connectWallet() {
  ...
  // Farcaster hosts may already have a wallet connected and may not always
  // show a prompt for eth_requestAccounts. So we:
  // 1) try eth_accounts first
  // 2) fallback to eth_requestAccounts
  // 3) surface friendly errors instead of throwing (prevents "black screen")
  try {
    const existing = await p.request({ method: "eth_accounts", params: [] });
    ...
    if (/user rejected|rejected/i.test(msg)) {
      ...
      toast("Wallet connect failed. Make sure a wallet is connected in your Farcaster client.");
