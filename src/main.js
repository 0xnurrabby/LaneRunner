...
async function connectWallet({ silent = false } = {}) {
  ...
  // In some Farcaster clients the wallet provider is injected a moment after `ready()`.
  // If we immediately call `eth_requestAccounts`, certain providers may redirect out
  // to a passkey / login flow (e.g. keys.coinbase.com). We retry `eth_accounts` briefly
  // first to give the in-app wallet time to populate accounts.
  if (!accs.length) {
    for (let i = 0; i < 3; i++) {
  ...
  if (!accs.length) {
    // Don't trigger an external login flow during silent auto-connect.
