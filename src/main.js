...
async function openComposerWithText(text, embeds = [HOME_URL]) {
  ...
  // Prefer native miniapp composer if available.
  try {
    if (s?.actions?.composeCast) {
      await s.actions.composeCast({ text: String(text || ""), embeds: Array.isArray(embeds) ? embeds : [embeds] });
      return true;
    }
  } catch {
    // fall through
  ...
  // Fallback: open Warpcast compose deep-link.
  const url = buildWarpcastComposeUrl(text, Array.isArray(embeds) ? embeds : [embeds]);
  try {
...
async function shareYourStats() {
  try {
    ...
    const uname = await getFcUsername().catch(() => null);
    const who = uname ? `@${uname}` : shortAddr(account);
...
async function openLeaderboardsView() {
  ...
  $("#shareStatsBtn").addEventListener("click", shareYourStats);
