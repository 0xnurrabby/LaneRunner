...
async function openComposerWithText(text, embeds = [HOME_URL]) {
  // Many in-app browsers block window.open after an await. Prefer SDK actions first,
  // and use location.href as the last resort.
  ...
  // 1) Native composer if supported (best UX inside Warpcast/Farcaster).
  if (s?.actions?.composeCast) {
    try {
      await s.actions.composeCast({ text: safeText, embeds: embedArr });
      return true;
    } catch {}
    // Some SDK builds don't accept embeds yet.
    try {
      await s.actions.composeCast({ text: safeText });
      return true;
    } catch {}
  }
  ...
  // 2) Ask the host to open the URL (works in Farcaster clients).
  if (s?.actions?.openUrl) {
    try {
      await s.actions.openUrl(url);
      return true;
    } catch {}
    // Some SDK builds accept an object instead of a string.
    try {
      await s.actions.openUrl({ url });
      return true;
    } catch {}
  ...
  // Some SDK builds call this openLink instead of openUrl.
  if (s?.actions?.openLink) {
    try {
      await s.actions.openLink(url);
      return true;
    } catch {}
    try {
      await s.actions.openLink({ url });
  ...
  // 3) Last resort: navigate the current webview.
  try {
...
async function shareYourStats() {
  try {
    ...
    const userId = await getShareUserId();
    const used = getDailyShareCount(userId);
    ...
    const opened = await openComposerWithText(text, [HOME_URL]);
    if (!opened) {
      toast("Could not open composer.");
      return;
    }
