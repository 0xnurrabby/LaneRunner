...
// Event ABI (server aggregates logs; kept here for reference / future use)
// NOTE: Current contract emits GameInteracted(address indexed user, bytes32 indexed action, uint256 timestamp, bytes data, string gameName, string message)
const ACTION_LOGGED_EVENT_ABI =
  "event GameInteracted(address indexed user, bytes32 indexed action, uint256 timestamp, bytes data, string gameName, string message)";
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    // IMPORTANT: The deployed contract at CONTRACT uses `interact(bytes32,bytes)`.
    // If this name doesn't match, the call will hit fallback() and revert ("<unknown>" in wallet).
...
async function openLeaderboardsView() {
  ...
  try {
    ...
    const allHtml = await renderList(allTop, { colorMode: "text" });
    const winnersHtml =
