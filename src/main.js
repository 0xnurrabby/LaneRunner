...
// LEADERBOARD API
// =====================================================
const BOARDS_CACHE_KEY = "w3r_boards_cache_v2";

function saveBoardsCache(payload) {
  try {
    // Store pts as strings (BigInt-safe)
    const ser = {
      savedAt: Date.now(),
      weekStart: payload.weekStart,
      prevWeekStart: payload.prevWeekStart,
      weeklySorted: (payload.weeklySorted || []).map((x) => ({
        addr: x.addr,
        pts: (typeof x.pts === "bigint" ? x.pts : BigInt(x.pts || 0)).toString(),
        name: typeof x.name === "string" ? x.name : shortAddr(x.addr)
      })),
      prevWeekSorted: (payload.prevWeekSorted || []).map((x) => ({
        addr: x.addr,
        pts: (typeof x.pts === "bigint" ? x.pts : BigInt(x.pts || 0)).toString(),
        name: typeof x.name === "string" ? x.name : shortAddr(x.addr)
      }))
    };
    localStorage.setItem(BOARDS_CACHE_KEY, JSON.stringify(ser));
  } catch {}
}

function loadBoardsCache(expectedWeekStart) {
  try {
    const raw = localStorage.getItem(BOARDS_CACHE_KEY);
    if (!raw) return null;
    const j = JSON.parse(raw);
    if (!j || Number(j.weekStart) !== Number(expectedWeekStart)) return null;
...
async function fetchLeaderboard(opts = {}) {
  ...
  const url = `/api/leaderboard?weekStart=${encodeURIComponent(String(weekStart))}${force ? "&force=1" : ""}`;
...
async function openLeaderboardsVielet boardsInFlight = false;
let boardsFetchPromise = null;

function isBoardsViewOpen() {
  return els.sheet.classList.contains("open") && els.sheet.dataset.view === "boards";
}

function renderLeaderboardsInto(containerEl, data, opts = {}) {
  const weekStart = data.weekStart;
  const prevWeekStart = data.prevWeekStart;
  const weeklySorted = data.weeklySorted || [];
  const prevWeekSorted = data.prevWeekSorted || [];
  ...
  const renderList = (items, opts2 = {}) => {
    const showFullAddr = !!opts2.showFullAddr;
    const colorMode = opts2.colorMode || null; // "text" | "row" | null

    return (items || [])
      .map((x, i) => {
        const nameStr = typeof x.name === "string" && x.name ? x.name : shortAddr(x.addr);
        ...
        const entryStyleAttr =
          colorMode === "row"
            ? ` style="
                --rowBg:${c.rowBg};
                --rowBg2:${c.rowBg2};
                --rowBorder:${c.rowBorder};
                --rowGlow:${c.rowGlow};
              "`
            : "";

        const styleAttr = colorMode === "text" ? ` style="--addrColor:${c.text};"` : "";
        ...
        return `
          <div class="entry${colorMode === "row" ? " entryRow" : ""}"${entryStyleAttr}>
            ...
            <div class="pts mono">${fmtPts(x.pts)}</div>
          </div>
        `;
      })
      .join("");
  ...
  const weeklyHtml = renderList(weeklyTop, { colorMode: "text" });
  const winnersHtml =
    lastWinners.length === 0
      ? `<div class="copy">No data found for last week yet.</div>`
      : renderList(lastWinners, { showFullAddr: true, colorMode: "row" });

  const statusLine = opts.updating
    ? `<div class="copy" style="opacity:.85;margin:6px 0 10px">Updating…</div>`
    : opts.fromCache
      ? `<div class="copy" style="opacity:.85;margin:6px 0 10px">Showing saved results.</div>`
      : ``;
  ...
  containerEl.innerHTML = `
    ...
    <div class="board">
      ...
      <div class="copy" style="margin-top:8px;opacity:.9">
        ${
          account
            ? yourWeeklyRank
    ...
    <div class="board">
      ...
      <div class="boardList">${winnersHtml}</div>
    </div>
  `;
...
function renderBoardsLoading(containerEl, msg = "Loading weekly leaderboards…") {
  containerEl.innerHTML = `
    <div class="copy" style="opacity:.9">${msg}</div>
    <div class="board" style="margin-top:10px">
      <div class="boardTitle"><div>Preparing results</div><div>…</div></div>
      <div class="boardList">
        ${new Array(8)
          .fill(0)
          .map(
            (_, i) => `
          <div class="entry entryRow" style="--rowBg:rgba(255,255,255,.06);--rowBg2:rgba(255,255,255,.03);--rowBorder:rgba(255,255,255,.10);--rowGlow:rgba(255,255,255,.05)">
            <div class="left">
              <div class="rankBadge">#${i + 1}</div>
              <div class="addr addrFull" style="width:140px;opacity:.45">██████████</div>
            </div>
            <div class="pts mono" style="opacity:.35">██████</div>
          </div>
        `
          )
          .join("")}
...
function ensureBoardsFetch(opts = {}) {
  const force = !!opts.force;

  if (force || !boardsFetchPromise) {
    boardsInFlight = true;
    boardsFetchPromise = fetchLeaderboard({ force })
      .catch((e) => {
        // Let caller render error; keep promise chain alive by rethrow
        throw e;
      })
      .finally(() => {
  ...
  return boardsFetchPromise;
}

function openLeaderboardsView() {
  openSheet(
    "Leaderboards",
    `
  ...
  $("#backMenu").addEventListener("click", openMainMenu);
  $("#refreshBoards").addEventListener("click", async () => {
    const el = $("#boards");
    renderBoardsLoading(el, "Refreshing…");
    try {
      const data = await ensureBoardsFetch({ force: true });
      if (isBoardsViewOpen()) renderLeaderboardsInto(el, data);
    } catch (e) {
      if (!isBoardsViewOpen()) return;
      el.innerHTML = `<div class="copy">Could not refresh right now. Try again.<br/><span class="mono">${String(
        e?.message || e
      )}</span></div>`;
    }
  });
  ...
  // Instant UX: show last saved boards immediately if available.
  const cached = loadBoardsCache(weekStartUtcMs());
  if (cached) {
    renderLeaderboardsInto(el, cached, { fromCache: true, updating: true });
  } else {
    renderBoardsLoading(el);
  }

  // Always kick off a background fetch when the view opens (acts like "refresh",
  // but still benefits from edge cache unless the user taps Refresh).
  ensureBoardsFetch({ force: false })
    .then((data) => {
      if (!isBoardsViewOpen()) return; // allow background warm-cache even if closed
      renderLeaderboardsInto(el, data);
    })
    .catch((e) => {
      // If we had cache, keep it and just show a small warning.
      if (!isBoardsViewOpen()) return;
      if (cached) {
        ...
        warn.innerHTML = `Network busy. Saved results shown. <span class="mono" style="opacity:.75">${String(
          e?.message || e
        ).slice(0, 180)}</span>`;
        el.prepend(warn);
        return;
      }
      el.innerHTML = `<div class="copy">Could not load on-chain logs. Try Refresh.<br/><span class="mono">${String(
        e?.message || e
      )}</span></div>`;
    });
}
