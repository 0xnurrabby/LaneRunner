...
function drawCarTopDown(x, y, w, h, bodyColor) {
  // Illustrated top-down car (closer to the reference image). No gameplay logic changes.
  ctx.save();

  // --- tiny helpers (local) ---
  function _parseColor(c) {
    // Supports rgba()/rgb() and #rgb/#rrggbb. Falls back to a nice red.
    if (typeof c !== "string") return { r: 220, g: 60, b: 70, a: 1 };
    let s = c.trim();
    ...
    return { r: 220, g: 60, b: 70, a: 1 };
  }
  function _mix(c, t, amt) {
    return {
      r: c.r + (t.r - c.r) * amt,
      g: c.g + (t.g - c.g) * amt,
      b: c.b + (t.b - c.b) * amt,
  ...
  function _rgba(c, aOverride) {
    const a = aOverride == null ? c.a : aOverride;
    return `rgba(${c.r|0},${c.g|0},${c.b|0},${a})`;
  }

  function _bodyPath() {
    // Curvy silhouette with slight mid-inset + wheel bulges
    const inset = w * 0.07;
    const bulge = w * 0.06;
    const topR = Math.min(w * 0.30, h * 0.20);
    const botR = topR;
    ...
    const xL = x + inset;
    const xR = x + w - inset;
    const xLOut = x + inset - bulge;
    const xROut = x + w - inset + bulge;

    ctx.beginPath();
    ctx.moveTo(x + w * 0.5, y0);
    // top-right curve
    ctx.quadraticCurveTo(x + w, y0, xR, y1);
    // upper wheel bulge
    ctx.quadraticCurveTo(x + w, y2, xROut, y2);
    // mid inset
    ctx.quadraticCurveTo(xR, y3, xR, y3);
    // lower wheel bulge
    ctx.quadraticCurveTo(x + w, y4, xROut, y4);
    // bottom-right curve
    ctx.quadraticCurveTo(x + w, y5, x + w * 0.5, y5);

    // mirror left side
    ctx.quadraticCurveTo(x, y5, xL, y4);
    ctx.quadraticCurveTo(x, y4, xLOut, y4);
    ctx.quadraticCurveTo(xL, y3, xL, y3);
    ctx.quadraticCurveTo(x, y2, xLOut, y2);
    ctx.quadraticCurveTo(x, y0, x + w * 0.5, y0);
    ctx.closePath();
  }
  ...
  ctx.beginPath();
  ctx.ellipse(x + w / 2, y + h * 0.92, w * 0.48, h * 0.16, 0, 0, Math.PI * 2);
  ctx.fillStyle = "rgba(0,0,0,0.8)";
  ctx.fill();
  ctx.globalAlpha = 1;

  const base = _parseColor(bodyColor);
  const light = _mix(base, { r: 255, g: 255, b: 255, a: 1 }, 0.24);
  const dark = _mix(base, { r: 0, g: 0, b: 0, a: 1 }, 0.40);
  const outline = _mix(base, { r: 0, g: 0, b: 0, a: 1 }, 0.58);

  // --- body fill (soft gradient) ---
  const paint = ctx.createLinearGradient(x, y, x + w, y);
  paint.addColorStop(0, _rgba(light, base.a));
  paint.addColorStop(0.48, _rgba(base, base.a));
  paint.addColorStop(1, _rgba(dark, base.a));

  _bodyPath();
  ctx.fillStyle = paint;
  ctx.fill();

  // --- outline like illustration ---
  ctx.lineWidth = Math.max(1.4, w * 0.065);
  ctx.strokeStyle = _rgba(outline, 0.60);
  ctx.stroke();
  ...
  // --- hood/roof shading (split tone, no center stripe) ---
  ctx.globalAlpha = 0.14;
  ctx.fillStyle = "rgba(0,0,0,1)";
  drawRoundedRect(x + w * 0.54, y + h * 0.08, w * 0.36, h * 0.84, Math.min(18, w * 0.30));
  ctx.fill();
  ctx.globalAlpha = 1;

  // --- windshield (front) ---
  const glass = ctx.createLinearGradient(x, y + h * 0.10, x, y + h * 0.44);
  glass.addColorStop(0, "rgba(10,18,30,0.78)");
  glass.addColorStop(1, "rgba(25,40,70,0.92)");
  ctx.fillStyle = glass;
  drawRoundedRect(x + w * 0.16, y + h * 0.12, w * 0.68, h * 0.26, Math.min(18, w * 0.26));
  ctx.fill();

  // windshield highlight
  ctx.globalAlpha = 0.35;
  const gh = ctx.createLinearGradient(x + w * 0.22, y + h * 0.14, x + w * 0.52, y + h * 0.36);
  gh.addColorStop(0, "rgba(255,255,255,0.55)");
  gh.addColorStop(1, "rgba(255,255,255,0.0)");
  ctx.fillStyle = gh;
  drawRoundedRect(x + w * 0.22, y + h * 0.15, w * 0.26, h * 0.20, Math.min(16, w * 0.22));
  ctx.fill();
  ctx.globalAlpha = 1;

  // --- rear window ---
  ctx.fillStyle = "rgba(18,26,45,0.78)";
  drawRoundedRect(x + w * 0.22, y + h * 0.68, w * 0.56, h * 0.17, Math.min(16, w * 0.22));
  ctx.fill();

  // --- side windows panels (gives real car feel) ---
  ctx.fillStyle = "rgba(12,18,32,0.70)";
  drawRoundedRect(x + w * 0.09, y + h * 0.30, w * 0.15, h * 0.34, Math.min(14, w * 0.18));
  drawRoundedRect(x + w * 0.76, y + h * 0.30, w * 0.15, h * 0.34, Math.min(14, w * 0.18));
  ctx.fill();

  // --- wheel arches / wheels (not boxy) ---
  const ww = Math.max(3.2, w * 0.13);
  const wh = Math.max(7.5, h * 0.16);
  const wy1 = y + h * 0.22;
  const wy2 = y + h * 0.62;

  ctx.fillStyle = "rgba(0,0,0,0.45)";
  drawRoundedRect(x - ww * 0.12, wy1, ww, wh, ww * 0.60);
  drawRoundedRect(x + w - ww * 0.88, wy1, ww, wh, ww * 0.60);
  drawRoundedRect(x - ww * 0.12, wy2, ww, wh, ww * 0.60);
  drawRoundedRect(x + w - ww * 0.88, wy2, ww, wh, ww * 0.60);
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(x + w * 0.24, y + h * 0.90, Math.max(1.8, w * 0.052), 0, Math.PI * 2);
  ctx.arc(x + w * 0.76, y + h * 0.90, Math.max(1.8, w * 0.052), 0, Math.PI * 2);
  ctx.fill();
  ctx.globalAlpha = 1;
...
function drawPlayerCarPremium(x, y, w, h) {
  // Player car uses the same illustrated silhouette, just a cleaner paint + glow.
  ctx.save();

  // Soft glow (keep it subtle so it doesn't look like a box)
  ctx.globalAlpha = 0.22;
  ctx.beginPath();
  ctx.ellipse(x + w / 2, y + h * 0.92, w * 0.56, h * 0.18, 0, 0, Math.PI * 2);
  ctx.fillStyle = "rgba(110,210,255,0.9)";
  ctx.fill();
  ctx.globalAlpha = 1;

  // Build a premium paint color as base
  const baseColor = "rgba(70,185,255,0.98)";

  // Reuse enemy car renderer but with a slightly richer finish
  // (No center stripe, no logic changes)
  drawCarTopDown(x, y, w, h, baseColor);

  // Extra top highlight like the reference illustration
  ctx.globalAlpha = 0.22;
  const hi = ctx.createLinearGradient(x + w * 0.20, y + h * 0.06, x + w * 0.55, y + h * 0.38);
  hi.addColorStop(0, "rgba(255,255,255,0.85)");
  hi.addColorStop(1, "rgba(255,255,255,0.0)");
  ctx.fillStyle = hi;
  drawRoundedRect(x + w * 0.14, y + h * 0.10, w * 0.30, h * 0.28, Math.min(18, w * 0.34));
  ctx.fill();
  ctx.globalAlpha = 1;
