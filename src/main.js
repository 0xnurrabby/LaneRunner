...
async function displayNameFor(addr) {
  try {
    ...
    if (raw) {
      ...
      const v = m?.[k];
      // Be defensive: older builds may have stored objects here.
      const s = _toStr(v);
      if (s) return s;
    }
  } catch {}
...
async function fetchLeaderboard(opts = {}) {
  ...
  const bust = opts.force ? `&bust=${Date.now()}` : "";
  const res = await fetch(`/api/leaderboard?weekStart=${encodeURIComponent(String(weekStart))}${bust}`);
  if (!res.ok) {
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    if (isSheetOpen()) await openLeaderboardsView({ force: true });
  } catch (e) {
...
function openMainMenu() {
  ...
  $("#btnLeaderboards").addEventListener("click", () => openLeaderboardsView({ force: true }));
...
function openHowView() {
  ...
  $("#goBoards").addEventListener("click", () => openLeaderboardsView({ force: true }));
}

// Leaderboards fetching: keep a single in-flight fetch so
// - opening the sheet behaves like "Refresh"
// - if the user closes/navigates away, the fetch can continue and populate cache
let _lbFetch = {
  weekStart: 0,
  startedAt: 0,
  promise: null
...
let _boardsViewSeq = 0;

function _ensureLbFetch(opts = {}) {
  const weekStart = weekStartUtcMs();
  const now = Date.now();
  const force = !!opts.force;

  const sameWeek = _lbFetch.weekStart === weekStart;
  const inFlight = !!_lbFetch.promise;

  // If a fetch is already running, reuse it for a short window to avoid hammering RPCs.
  if (inFlight && sameWeek) {
    // If the user is spamming open/close, reuse for ~10s.
    if (now - _lbFetch.startedAt < 10000) return _lbFetch.promise;
    // If not forcing, reuse up to ~30s.
    if (!force && now - _lbFetch.startedAt < 30000) return _lbFetch.promise;
  }

  _lbFetch.weekStart = weekStart;
  _lbFetch.startedAt = now;
  // Only force-bust when requested. Otherwise allow edge/local caches to help.
  _lbFetch.promise = fetchLeaderboard({ force }).finally(() => {
    // Keep the promise object around for reuse within the windows above,
    // but allow a new fetch after enough time passes.
  });
  return _lbFetch.promise;
}
...
function renderLeaderboardsLoading() {
  // A lightweight, professional loader: racing track + skeleton lists.
  // This keeps users entertained while the on-chain scan runs.
  return `
    <div class="lbLoading" aria-live="polite">
      <div class="lbHero">
        <div class="lbRace" role="img" aria-label="Loading animation">
          <div class="lbTrack">
            <div class="lbLane"></div>
            <div class="lbLane"></div>
            <div class="lbLane lbLaneSoft"></div>

            <div class="lbCarWrap lbCarWrap1"><div class="lbCar lbCar1" aria-hidden="true"></div></div>
            <div class="lbCarWrap lbCarWrap2"><div class="lbCar lbCar2" aria-hidden="true"></div></div>
            <div class="lbCarWrap lbCarWrap3"><div class="lbCar lbCar3" aria-hidden="true"></div></div>
            <div class="lbCarWrap lbCarWrap4"><div class="lbCar lbCar4" aria-hidden="true"></div></div>
          </div>
        </div>
        <div class="lbHint">
          <span id="lbHintText">Indexing weekly results</span><span class="lbDots" aria-hidden="true"></span>
        </div>
        <div class="lbMini">Tip: Save often to protect your Bank.</div>
      </div>

      <div class="lbSkeleton">
        <div class="lbSkSection">
          <div class="lbSkTitle shimmer"></div>
          <div class="lbSkRows">
            ${Array.from({ length: 6 })
              .map(
                () => `
              <div class="lbSkRow shimmer">
                <div class="lbSkBadge"></div>
                <div class="lbSkName"></div>
                <div class="lbSkPts"></div>
              </div>`
              )
              .join("")}
          </div>
        </div>
        <div class="lbSkSection" style="margin-top:12px">
          <div class="lbSkTitle shimmer" style="width:62%"></div>
          <div class="lbSkRows">
            ${Array.from({ length: 4 })
              .map(
                () => `
              <div class="lbSkRow shimmer">
                <div class="lbSkBadge"></div>
                <div class="lbSkName" style="width:72%"></div>
                <div class="lbSkPts" style="width:18%"></div>
              </div>`
              )
              .join("")}
          </div>
...
let _lbHintTimer = 0;
function startLeaderboardsHintCycler() {
  clearInterval(_lbHintTimer);
  const el = document.getElementById("lbHintText");
  const tip = document.querySelector(".lbMini");
  if (!el || !tip) return;
  const hints = [
    "Indexing weekly results",
    "Reading Base logs",
    "Ranking top players",
    "Finalizing leaderboard"
  ];
  const tips = [
    "Tip: Save often to protect your Bank.",
    "Tip: Coins convert into points from the Menu.",
    "Tip: Commit adds your Bank to the weekly board.",
    "Tip: Boosts help you survive longer."
  ];
  let i = 0;
  _lbHintTimer = setInterval(() => {
    i = (i + 1) % hints.length;
...
function stopLeaderboardsHintCycler() {
  clearInterval(_lbHintTimer);
  _lbHintTimer = 0;
}

async function openLeaderboardsView(opts = {}) {
  const viewSeq = ++_boardsViewSeq;

  openSheet(
    "Leaderboards",
    `
    <div class="copy" id="boardsStatus" style="opacity:.9">Loading on-chain leaderboard…</div>
    <div class="btnRow" style="margin-top:12px">
      <button class="pill" id="backMenu">Back</button>
      <button class="pill" id="refreshBoards">Refresh</button>
    </div>
    <div id="boards" class="boardsWrap">${renderLeaderboardsLoading()}</div>
  `,
    "boards"
  );

  $("#backMenu").addEventListener("click", openMainMenu);
  $("#refreshBoards").addEventListener("click", () => openLeaderboardsView({ force: true }));

  // Start the loader text/tip cycler (will be replaced once real data renders)
  startLeaderboardsHintCycler();

  const cacheKey = `lb_season_${weekStartUtcMs()}`;
  ...
  const readCached = () => {
    try {
      ...
      // Minimal shape check
      if (!j || !Array.isArray(j.weeklySorted) || !Array.isArray(j.prevWeekSorted)) return null;
      if (typeof j._cachedAt !== "number") j._cachedAt = 0;
      return j;
    } catch {
      return null;
    }
...
const readAnyCached = () => {
  try {
    // If this is the first time opening this week (no cache yet),
    // show the most recent saved leaderboard from any week immediately.
    const keys = Object.keys(localStorage || {}).filter((k) => k && k.startsWith("lb_season_"));
    let best = null;
    let bestAt = 0;
    for (const k of keys) {
      ...
      if (!raw) continue;
      const j = JSON.parse(raw);
      if (!j || !Array.isArray(j.weeklySorted) || !Array.isArray(j.prevWeekSorted)) continue;
      const t = Number(j._cachedAt || 0);
      if (t > bestAt) {
        bestAt = t;
        best = j;
      }
...
const writeCached = (data) => {
  ...
  const stillBoardsView = () => {
    // If the user navigated away, the DOM node can be gone.
    // Also ignore updates from older views.
    if (viewSeq !== _boardsViewSeq) return false;
    return !!document.getElementById("boards") && els.sheet?.dataset?.view === "boards";
  };

  try {
    let cached = readCached();
if (!cached) {
  const anyCached = readAnyCached();
  if (anyCached && stillBoardsView()) {
    // Show *something* instantly so users aren't staring at a blank loader
    // on first open of the week. We'll still fetch the real weekly data in background.
    const st = document.getElementById("boardsStatus");
    if (st) st.textContent = "Loading leaderboard… (showing last saved)";
    await renderLeaderboards(anyCached, { stale: true });
  }
}

    if (cached && stillBoardsView()) {
      // Render instantly from last successful response; update in background.
      const st = document.getElementById("boardsStatus");
      if (st) st.textContent = "Showing saved leaderboard… updating";
      await renderLeaderboards(cached, { stale: true });
    }

    // Fetch in background. If we have a recent cached result, don't force-bust
    // so edge caching can keep the UI fast. The Refresh button always forces.
    const cachedAgeMs = cached ? Math.max(0, Date.now() - Number(cached._cachedAt || 0)) : Infinity;
    const shouldForce = !!opts.force || !cached || cachedAgeMs > 20 * 60 * 1000;
    const data = await _ensureLbFetch({ force: shouldForce });
    writeCached(data);

    // If user closed/navigated away, don't try to render—just keep the cache warm.
    if (stillBoardsView()) {
      await renderLeaderboards(data, { stale: false });
    }
  } catch (e) {
    if (stillBoardsView()) {
      const boardsEl = document.getElementById("boards");
      if (boardsEl) {
        boardsEl.innerHTML = `<div class="copy">Could not load on-chain logs. Try Refresh. ${
  ...
  async function renderLeaderboards(data, flags = {}) {
    // Once we have something to show (cached or fresh), stop the loader hint cycler.
    stopLeaderboardsHintCycler();

    const st = document.getElementById("boardsStatus");
    if (st) {
      st.textContent = flags.stale ? "Showing saved leaderboard… updating" : "Leaderboard updated";
    }
    ...
    const staleNote = flags.stale
      ? `<div class="copy" style="opacity:.75;margin-bottom:8px">Showing cached results… updating.</div>`
      : "";

    $("#boards").innerHTML = `${staleNote}
      <div class="board">
