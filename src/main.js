...
function _ensureLbFetch(opts = {}) {
  ...
  // Still "feels" like refresh because we always bust caches for that promise.
  ...
  // Always force-bust caches so "open leaderboard" acts like refresh.
  _lbFetch.promise = fetchLeaderboard({ force: true }).finally(() => {
...
async function openLeaderboardsView(opts = {}) {
  ...
  try {
    ...
    // IMPORTANT: opening the leaderboards should behave like pressing Refresh.
    // We always fetch with cache-bust, but we reuse an in-flight promise to avoid RPC spam.
    const data = await _ensureLbFetch({ force: true });
    writeCached(data);
