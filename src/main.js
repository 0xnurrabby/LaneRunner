...
function setupAudio() {
  ...
  AUDIO.bgm.playsInline = true;
  try { AUDIO.bgm.load(); } catch {}
  AUDIO.bgm.loop = true;
...
function playCoinSfx() {
  ...
  AUDIO.coinIdx++;
  try {
    // Avoid slow seeks on some mobile browsers
    if (typeof a.fastSeek === "function") a.fastSeek(0);
    else if (a.readyState >= 2) a.currentTime = 0;
    a.play().catch(() => {});
  } catch {}
}
...
// Farcaster identity (if running inside Farcaster Mini App)
let fid = 0n;
try {
  const maybeFid = await getFcFid();
  const n = Number(maybeFid);
  if (Number.isFinite(n) && n > 0) fid = BigInt(n);
} catch {}

const payload = encodeAbiParameters(
  [{ type: "uint256" }, { type: "uint256" }, { type: "uint256" }],
  [BigInt(pts), BigInt(weekStart), fid]
);
...
const RENDER_CACHE = { bg: null, key: "" };

function buildBackground(g, key) {
  const c = document.createElement("canvas");
  c.width = g.w;
  c.height = g.h;
  const bctx = c.getContext("2d");

  function drawRoundedRectOn(x, y, w, h, r) {
    const rr = Math.min(r, w / 2, h / 2);
    bctx.beginPath();
    bctx.moveTo(x + rr, y);
    bctx.lineTo(x + w - rr, y);
    bctx.quadraticCurveTo(x + w, y, x + w, y + rr);
    bctx.lineTo(x + w, y + h - rr);
    bctx.quadraticCurveTo(x + w, y + h, x + w - rr, y + h);
    bctx.lineTo(x + rr, y + h);
    bctx.quadraticCurveTo(x, y + h, x, y + h - rr);
    bctx.lineTo(x, y + rr);
    bctx.quadraticCurveTo(x, y, x + rr, y);
    bctx.closePath();
  }

  // Background
  bctx.fillStyle = "#0b0b10";
  bctx.fillRect(0, 0, g.w, g.h);

  const grd = bctx.createRadialGradient(g.w / 2, g.h / 2, 40, g.w / 2, g.h / 2, g.w);
  ...
  drawRoundedRectOn(g.roadX, g.roadY, g.roadW, g.roadH, 26);
  bctx.fill();
  ...
  for (let i = 1; i < g.lanes; i++) {
    ...
    bctx.beginPath();
    bctx.moveTo(x, g.roadY + 18);
    bctx.lineTo(x, g.roadY + g.roadH - 18);
    bctx.stroke();
  }
  bctx.setLineDash([]);
  bctx.restore();
...
function render() {
  ...
  // Cache static background (gradient + road + lane lines) to reduce jank on mobile
  const key = `${g.w}|${g.h}|${g.roadX}|${g.roadY}|${g.roadW}|${g.roadH}|${g.lanes}`;
  if (!RENDER_CACHE.bg || RENDER_CACHE.key !== key) buildBackground(g, key);

  ctx.clearRect(0, 0, g.w, g.h);
  ctx.drawImage(RENDER_CACHE.bg, 0, 0);
