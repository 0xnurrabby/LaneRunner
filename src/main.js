...
async function openLeaderboardsView() {
  ...
  try {
    ...
    const paletteForIndex = (i) => {
      ...
      return {
        text: `hsl(${hue.toFixed(1)}, ${sat}%, 42%)`,
        bg: `hsl(${hue.toFixed(1)}, ${sat}%, 83%)`
      };
    };

    const renderList = async (items, opts = {}) => {
      const showFullAddr = !!opts.showFullAddr;
      const colorMode = opts.colorMode || null; // "text" | "bg" | null
    ...
    const winnersHtml =
      lastWinners.length === 0
        ...
        : `<div class="boardList">${await renderList(lastWinners, { showFullAddr: true, colorMode: "bg" })}</div>`;
