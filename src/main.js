...
async function updateShareCounterUI() {
  ...
  if (btn) {
    // Keep the button usable even if the leaderboard request fails.
    // We only disable when we *know* the daily limit is reached.
...
function openComposerImmediate(text, embeds = [HOME_URL]) {
  const s = _getSdkSync() || sdk;

  const safeText = String(text || "");
  const embedArr = Array.isArray(embeds) ? embeds.filter(Boolean).map((e) => String(e)) : [String(embeds)];

  // Prefer native composeCast if available (recommended by Farcaster Mini Apps docs).
  if (s?.actions?.composeCast) {
    try {
      return Promise.resolve(s.actions.composeCast({ text: safeText, embeds: embedArr }))
        .then(() => true)
        .catch(() => false);
    } catch {
  ...
  if (s?.actions?.openUrl) {
    try {
      return Promise.resolve(s.actions.openUrl(url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
  ...
  if (s?.actions?.openLink) {
    try {
      return Promise.resolve(s.actions.openLink(url))
        .then(() => true)
        .catch(() => false);
    } catch {}
    try {
...
function shareYourStatsFromGesture() {
  try {
    ...
    // If the leaderboard hasn't loaded, we still let the user share something useful
    // (and keep the button responsive). We'll include bank points and the app link.
    const hasBoards = !!lastLeaderboardData;
    const weeklySorted = hasBoards ? lastLeaderboardData.weeklySorted : [];
    const prevWeekSorted = hasBoards ? lastLeaderboardData.prevWeekSorted : [];
    const allTimeSorted = hasBoards ? lastLeaderboardData.allTimeSorted : [];
    const weekStart = hasBoards ? lastLeaderboardData.weekStart : weekStartUtcMs();
    const prevWeekStart = hasBoards ? lastLeaderboardData.prevWeekStart : weekStartUtcMs() - 7 * 24 * 60 * 60 * 1000;

    const w = hasBoards && account ? findRankAndPts(weeklySorted, account) : { rank: null, pts: 0n };
    const p = hasBoards && account ? findRankAndPts(prevWeekSorted, account) : { rank: null, pts: 0n };
    const a = hasBoards && account ? findRankAndPts(allTimeSorted, account) : { rank: null, pts: 0n };

    const who = fcUsername ? `@${fcUsername}` : (account ? shortAddr(account) : "Anon");
    ...
    const lines = [
      ...
      "",
      hasBoards && account ? fmtLine("All-time", a) : `All-time: ${account ? "(loading…)" : "Connect wallet to show"}`,
      hasBoards && account ? fmtLine(`This week (since ${weekLabel} UTC)`, w) : `This week: ${account ? "(loading…)" : "Connect wallet to show"}`,
      hasBoards && account ? fmtLine(`Last week (since ${prevLabel} UTC)`, p) : `Last week: ${account ? "(loading…)" : "Connect wallet to show"}`,
      "",
      `Bank points: ${Math.floor(profile.bankPoints)}`,
      account ? "" : "Tip: connect wallet for leaderboard ranks.",
      "",
      "Play:",
      HOME_URL
    ].filter((x) => x !== "");
    // Keep text reasonably short for better host compatibility.
...
async function openLeaderboardsView() {
  ...
  try {
    ...
    // Keep share usable even if leaderboard fails to load.
    // (Daily limit is still enforced by updateShareCounterUI)
    updateShareCounterUI();
