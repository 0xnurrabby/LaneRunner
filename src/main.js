...
async function renderStatus() {
  ...
  if (fcUser && fcUser.username) {
    const avatar = fcUser.pfpUrl || "";
    els.statusBadge.innerHTML = avatar
      ? `<img src="${avatar}" style="width:18px;height:18px;border-radius:50%;vertical-align:middle;margin-right:6px"/>@${fcUser.username}`
      : `@${fcUser.username}`;
    return;
  }
  if (!account) {
