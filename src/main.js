// Keep the initial load *very* light.
// Heavy web3 libs were previously imported at module-top (viem + ox/erc8021),
// which makes the Mini App "Open" feel slow because the browser must fetch &
// evaluate big ESM bundles before *any* UI can appear.
//
// Strategy:
// - Only load Farcaster SDK up-front (small).
// - Replace the tiny subset of ABI encoding we need with lightweight helpers.
// - Lazy-load ox/erc8021 only when the user actually commits on-chain.
...
// On-chain action name as bytes32 (right-padded ASCII): bytes32("WEEKLY_ADD")
// Precomputed to avoid pulling a large ABI library just for this.
const ACTION_WEEKLY_ADD =
  "0x5745454b4c595f41444400000000000000000000000000000000000000000000";

// Lazy-load Base Builder attribution suffix ONLY when needed for on-chain commit.
let _dataSuffixPromise = null;
async function getDataSuffix() {
  if (_dataSuffixPromise) return _dataSuffixPromise;
  _dataSuffixPromise = import("https://esm.sh/ox/erc8021")
    .then((m) => {
      const Attribution = m?.Attribution;
      if (!Attribution?.toDataSuffix) throw new Error("Attribution unavailable");
      return Attribution.toDataSuffix({ codes: [BUILDER_CODE] });
    })
    .catch((e) => {
      console.warn("Failed to load attribution suffix", e);
      return null;
    });
  return _dataSuffixPromise;
}

// --- Minimal ABI helpers (fast + dependency-free) ---
// We only need to encode:
//   payload = abi.encode(uint256 points, uint256 weekStartMs)
//   calldata = logAction(bytes32 action, bytes payload)
// Selector for logAction(bytes32,bytes) = 0x2d9bc1fb
const SELECTOR_LOG_ACTION = "0x2d9bc1fb";
...
function uint256ToWordHex(n) {
  const b = typeof n === "bigint" ? n : BigInt(n);
  if (b < 0n) throw new Error("uint256 underflow");
  return pad32Hex(b.toString(16));
}

function encodeUint256Pair(points, weekStartMs) {
  const a = uint256ToWordHex(points);
  const b = uint256ToWordHex(weekStartMs);
  return "0x" + a + b;
}
...
function encodeBytesDynamic(bytesHex) {
  ...
  const lenWord = uint256ToWordHex(len);
  // pad data to 32-byte boundary
  const pad = (64 - (data.length % 64)) % 64;
  const padded = data + "0".repeat(pad);
  return { lenWord, padded };
}

function buildLogActionCalldata(actionBytes32Hex, payloadHex) {
  const action = strip0x(actionBytes32Hex);
  if (action.length !== 64) throw new Error("action must be bytes32");

  const payload = strip0x(payloadHex);
  const { lenWord, padded } = encodeBytesDynamic(payload);

  // Head: action (32) + offset (32) -> offset is 0x40 because 2 args
  const headAction = action;
  const headOffset = uint256ToWordHex(0x40);
  const tail = lenWord + padded;

  return SELECTOR_LOG_ACTION + headAction + headOffset + tail;
}
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    const weekStart = weekStartUtcMs();
    // payload = abi.encode(uint256 points, uint256 weekStartMs)
    const payload = encodeUint256Pair(BigInt(pts), BigInt(weekStart));

    // calldata = logAction(bytes32 action, bytes payload)
    const data = buildLogActionCalldata(ACTION_WEEKLY_ADD, payload);

    const dataSuffix = await getDataSuffix();
    if (!dataSuffix) throw new Error("Attribution unavailable");
    ...
    const renderList = async (items) => {
      const rows = await Promise.all(
        items.map(async (x, i) => {
          ...
          const isFullAddr = /^0x[a-fA-F0-9]{40}$/.test(String(name || ""));
          return `
            <div class="entry">
              <div class="left">
                <div class="rankBadge">#${i + 1}</div>
                <div class="addr${isFullAddr ? " addrFull" : ""}">${name}</div>
              </div>
