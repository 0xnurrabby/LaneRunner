...
async function fetchLeaderboard() {
  ...
  for (const l of logs) {
    ...
    if (Number(wk) === weekStart) {
      const prevW = weekly.get(user) || 0n;
      weekly.set(user, prevW + points);
    }
  }

  return {
    weekStart,
    weeklyTop: sortTop(weekly),
    allTimeTop: sortTop(allTime)
  };
}
...
async function openLeaderboardsView() {
  ...
  try {
    ...
    const weeklyHtml = data.weeklyTop.length
      ? data.weeklyTop
          .map(
            (r, i) =>
              `<div class="row"><span>#${i + 1}</span><span>${shortAddr(r.addr)}</span><b>${Number(r.pts)}</b></div>`
          )
          .join("")
      : `<div class="copy" style="opacity:.85">No weekly commits yet.</div>`;

    const allHtml = data.allTimeTop.length
      ? data.allTimeTop
          .map(
            (r, i) =>
              `<div class="row"><span>#${i + 1}</span><span>${shortAddr(r.addr)}</span><b>${Number(r.pts)}</b></div>`
          ...
          .join("")
      : `<div class="copy" style="opacity:.85">No commits yet.</div>`;

    const weekLabel = new Date(data.weekStart).toISOString().slice(0, 10);

    $("#boards").innerHTML = `
      <div class="board">
        <div class="boardTitle">Weekly (since ${weekLabel} UTC)</div>
        <div class="boardList">${weeklyHtml}</div>
      </div>
      <div class="board" style="margin-top:12px">
        <div class="boardTitle">All-time</div>
        <div class="boardList">${allHtml}</div>
      </div>
    `;
  } catch {
    $("#boards").innerHTML = `<div class="copy">Could not load logs yet. Try Refresh.</div>`;
  }
}
