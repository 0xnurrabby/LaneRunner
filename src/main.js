...
let fcUsername = null; // handle (without @)
let fcDisplayName = null;
let fcFid = null;
...
function _extractFcUser(ctx) {
  if (!ctx) return null;
  if (ctx.user) return ctx.user;
  if (ctx.context?.user) return ctx.context.user;
  // Some clients nest differently; handle the common "context" shape.
  if (ctx.context && (ctx.context.fid || ctx.context.username || ctx.context.displayName)) return ctx.context;
  return null;
}

function _cleanHandle(s) {
  if (!s) return null;
...
function getFcLabel() {
  if (fcUsername) return `@${fcUsername}`;
  if (fcDisplayName) return fcDisplayName;
  return null;
}

async function getFcUsername({ retries = 20, delayMs = 250 } = {}) {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      const ctx = (sdk && sdk.context) || null;
      const u = _extractFcUser(ctx);

      const uname = _cleanHandle(_toStr(u?.username ?? null));
      const dname = _toStr(u?.displayName ?? u?.name ?? null);
      const fid = u?.fid ?? ctx?.fid ?? ctx?.userFid ?? null;

      if (uname || dname) {
        if (uname) fcUsername = uname;
        if (dname) fcDisplayName = dname;
        if (fid != null) fcFid = fid;
...
async function hydrateFarcasterLabel() {
  // Best-effort: try to populate local display name mapping for the connected address.
  try {
    await getFcUsername({ retries: 20, delayMs: 250 });
    const label = getFcLabel();
    if (label && account) {
      const raw = localStorage.getItem("addrNameMap");
      const m = raw ? JSON.parse(raw) : {};
      m[String(account).toLowerCase()] = label;
...
function openMainMenu() {
  ...
  if (account) {
    try {
      const raw = localStorage.getItem("addrNameMap");
      const m = raw ? JSON.parse(raw) : {};
      walletLine = m?.[String(account).toLowerCase()] || shortAddr(account);
    } catch {
      walletLine = shortAddr(account);
    }
  }
