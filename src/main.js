...
async function fetchLeaderboard() {
  ...
  const qs = `weekStart=${encodeURIComponent(String(weekStart))}&names=0`;

  // In some Mini App environments, relative fetch can be flaky depending on
  // how the app is embedded. Try same-origin first, then fall back to HOME_URL.
  let res;
  try {
    res = await fetch(`/api/leaderboard?${qs}`, { cache: "no-store" });
  } catch {
    const abs = new URL(`/api/leaderboard?${qs}`, HOME_URL).toString();
